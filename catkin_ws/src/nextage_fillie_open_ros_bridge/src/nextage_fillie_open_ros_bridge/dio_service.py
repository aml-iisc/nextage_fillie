...
def main():
    ...
    rospy.init_node("nextage_fillie_open_hand_dio")
    ...
    def make_on_off_service(name):
        ...
        def off_handler(req):
            ...
            return EmptyResponse()
    	on = rospy.Service("nextage_fillie_open_hand_dio/" + name + "/on", Empty, on_handler)
    	off = rospy.Service("nextage_fillie_open_hand_dio/" + name + "/off", Empty, off_handler)
        return on, off
    ...
    def make_empty_service(name):
        def handler(req):
            getattr(robot, name)()
            return EmptyResponse()
        return rospy.Service("nextage_fillie_open_hand_dio/" + name, Empty, handler)
