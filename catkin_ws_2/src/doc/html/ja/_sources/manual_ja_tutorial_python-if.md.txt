...
## RTM Python インタフェース<!--RTM Python interfaces-->

### インタラクティブモードでの操作<!--Interactive mode of operation-->

#### MoveIt の起動<!--Starting MoveIt-->

「Move It!」の起動を実施しておきます。

```
$ roslaunch nextage_fillie_open_moveit_config moveit_planning_execution.launch
```

#### rtm_ros_bridge の起動<!--Starting rtm_ros_bridge-->
...
#### iPython インタフェースの起動<!--Starting the iPython interfaces-->


NEXTAGE OPEN での iPython インタフェースの起動を起動します。
...
```
iPython インタフェース初期化時には次のことを行っています。
...
#### よく使う RTM Python インタフェースコマンド<!--Common RTM Python interface commands-->

ロボット実機操作において特有なコマンドを紹介します。

- 注意-1: コマンドを実行するとロボットが動きます。
- 注意-2: 非常停止スイッチをいつでも押せる状態にしておいてください。
...
- ロボット実機の再キャリブレーションが必要な場合(LED青点滅)にのみ実行します。
...
- 次の動作を行ってサーボが切れた状態のときには上記コマンド等によりサーボを入れる必要があります。
  - 非常停止スイッチを押したとき（この場合はまず `servoOff` を行う必要があります）
  - ロボットが自己診断で緊急停止したとき
- 関節角度と現在の角度指示値が異なっている場合に上記コマンドでサーボを入れると、ロボットが指示値に向かって急激に動作を行います。サーボを入れる直前に以下の操作の内どちらかを必ず実行してください。
  - DiagnosisUIの[Suspend Resume]ボタンを押す。
  - `robot.goActual()`を実行する。
...
- サーボを切るのではなく非常停止スイッチを押した場合にはこのコマンドを実行してください。

#### RTM Python インタフェースの利用<!--Using RTM Python Interfaces-->

iPython の初期化時にロボットのクライアントインタフェースクラスである NextageFillieOpenClient が iPython ターミナル上で `robot` としてインスタンス化されます。
...
この内 `getCurrentPose` が意図するコマンドであることが分かります。
...
```
In [0] : robot.getCurrentPose('RARM_JOINT0')
Out:
[1.0,
...
#### hrpsys-based API のソースとドキュメント<!--hrpsys-based API sources and documents-->
...
### RTM Python インタフェースプログラミング<!--RTM Python interface programming-->

Python を用いた HIRO / NEXTAGE Fillie OPEN のプログラミングは1つの統合されたインタフェースによって行うことができます。
インタフェースの名称はロボットによりそれぞれ異なります。

- HIRONX : HIRO / HIRONX ユーザ
- NextageFillieOpenClient : NEXTAGE Fillie OPEN ユーザ

本項では "HIRONX" を使用しますが NEXTAGE Fillie OPEN ユーザは NextageFillieOpenClient インタフェースを用いて同様のことを行うことができます。

#### サンプルコード - Acceptance Test (RTM)<!--Sample Code-Acceptance Test (RTM)-->

Acceptance Test (RTM) のコードを参考に RTM Python インタフェースプログタミング
の方法を見たあとにコードを実行します。

##### acceptancetest_rtm.py<!--acceptancetest_rtm.py-->

このサンプルは下図のような2段階の依存関係を持っています。
...
RTM API の HIRONX もしくは NextageFillieOpenClient クラスがインポートされていません。しかし、次の部分を見ると、
...
##### サンプルコードの実行<!--Sample code execution-->
...
iPython インタフェースを終了するときは `Ctrl-d` にてエスケープします。


#### サンプルコード - 円を描く<!--Sample code-draw a circle-->
...
以下のコマンドで実行してください。ここで、%HOST%はロボットのIPアドレスを指します。
```
$ ipython -i `rospack find nextage_fillie_open_ros_bridge`/script/nextage_fillie_open.py -- --host %HOST% (実機)
...
#### 使用場面に応じたプログラミング<!--Programming according to the situation of use-->

デフォルトではいくつかの HIRONX / NextageFillieOpenClient クラスのコマンドは
...
この場合 `HrpsysConfigurator` の `waitInterpolationOfGroup()` を呼び出して対処することができます。
...
#### 連続した軌道座標指示による動作<!--Motion with continuous trajectory coordinate designation-->
...
一時停止から復帰直後に`playPatternOfGroup`を実行すると急激にロボットが動作することがあります。一つ目の通過点として移動目標に現在の関節角度、動作時間に0.001を指定すると安全に動作させることができます。  
...
#### 相対姿勢指示による動作<!--Relative attitude indication-->

エンドエフェクタのフレームや関節を現在の姿勢からの相対姿勢動作は、
NextageFillieOpenClient インタフェースの `setTargetPoseRelative` を用いることで簡単に行うことができます。  
注）実機において条件によっては想定と異なる姿勢に到達する場合を確認しています。
...
#### ユーザ Python コードの作成<!--Creating User Python Codes-->

ここまでの RTM Python インタフェースのチュートリアルでは `hironx.py` や `nextage_fillie_open.py` といったスクリプトを実行していました。

クライアントインタフェースクラスのメソッドを利用して
アプリケーションモジュールを作成する方法を考えてみます。
そのモジュール名を `your_nxo_sample.py` とし、
シミュレーションや実機で動作させるものとします。

1. まず1つの方法として `nextage_fillie_open.py` を複製して `NextageFillieOpenClient` クラスがインスタンス化される行を書き換えます。
他の `NextageFillieOpenClient` クラスがアクセスする部分も削除や書き換えを行います。
...
### デジタルI/O の利用（NEXTAGE Fillie OPEN）<!--Use of digital I/O (NEXTAGE Fillie OPEN)-->

本項はデジタルI/O（DIO）が備わっている NEXTAGE Fillie OPEN でのみ有効です。

DIO 操作で利用できるメソッドについては、特にデフォルトで利用可能なメソッドが集められている `NextageFillieOpenClient` クラスのコードを参照してください。

- コード: ~/catkin_ws/src/nextage_fillie_open_ros_bridge/src/nextage_fillie_open_ros_bridge/nextage_fillie_open_client.py
- 参考: 旧式NEXTAGE OPENのドキュメント  
※ パッケージ名やクラス名などが異なりますが基本構造は変わりません。
  - APIドキュメント:
  [http://docs.ros.org/en/melodic/api/nextage_ros_bridge/html/annotated.html](http://docs.ros.org/en/melodic/api/nextage_ros_bridge/html/annotated.html)
  - NextageClient: [http://docs.ros.org/en/melodic/api/nextage_ros_bridge/html/  
  classnextage__ros__bridge_1_1nextage__client_1_1NextageClient.html](http://docs.ros.org/en/melodic/api/nextage_ros_bridge/html/classnextage__ros__bridge_1_1nextage__client_1_1NextageClient.html)

#### RTM クライアントからのインタラクティブな操作<!-- Interactive Operations from RTM Clients-->

まず RTM クライアントを実行し、ネームサーバー内部（つまりコントローラのQNX上）のロボットに接続する必要があります。

```
$ ipython -i `rospack find nextage_fillie_open_ros_bridge`/script/nextage_fillie_open.py -- --host %HOST%
```

上記により、 `_hands` オブジェクトから全ての DIO メソッドにアクセスすることができます。
...
※ デフォルトで`_hands`オブジェクトの機能は無効化されています。有効化する場合には、関数`__init__`内にある`set_hand_version`の引数を変更してください。`BaseHands`を継承したクラスを作成し、`set_hand_version`に追加することで任意のハンドに対応できます。  
  
ハンドツールを接続・取り外しする場合は次のように行います。  
※ 参照ドキュメントは旧式NEXTAGE OPENのものですが、パッケージ名やクラス名などが異なるのみで基本構造は変わりません。
...
他のよく使われるメソッドについては以下のようになります。

- グリッパの開閉: `gripper_l_open`, `gripper_r_open`, `gripper_l_close`, `gripper_r_close`
- ライトの点灯等: `handlight_both`, `handlight_l`, `handlight_r` (現在のモデルだと対応していません)
